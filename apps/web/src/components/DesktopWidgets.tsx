...
// ─── Shared style objects (reference CSS variables) ───────────────────────────
...
const widgetHeaderBorder: React.CSSProperties = {
  borderBottom: '1px solid var(--widget-header-border)',
}
...
function Widget({
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className="flex items-center justify-between px-3.5 py-2.5" style={widgetHeaderBorder}>
...
const STATUS_COLOR: Record<string, string> = {
  ...
  completed: 'rgba(74,222,128,0.10)',
  failed:    'rgba(248,113,113,0.12)',
  pending:   'rgba(250,204,21,0.10)',
}
const STATUS_TEXT: Record<string, string> = {
  running: '#3b82f6', completed: '#22c55e', failed: '#ef4444', pending: '#eab308',
}
...
function SystemOverviewWidget() {
  ...
  const stats = [
    { label: 'Agents',   value: agents.length,    icon: <Bot className="w-4 h-4" />,                                           color: '#c084fc', app: 'agents' },
    { label: '运行中',    value: running,           icon: <Loader2 className={`w-4 h-4 ${running > 0 ? 'animate-spin' : ''}`} />, color: '#60a5fa', app: 'tasks' },
    { label: '待处理',    value: pending,           icon: <Clock className="w-4 h-4" />,                                          color: '#fbbf24', app: 'tasks' },
    { label: '已完成',    value: completed,         icon: <CheckCircle2 className="w-4 h-4" />,                                   color: '#34d399', app: 'tasks' },
    { label: 'Workflows', value: workflows.length,  icon: <Workflow className="w-4 h-4" />,                                        color: '#fb923c', app: 'canvas' },
  ]
  ...
  return (
    ...
    >
      {stats.map((s, i) => (
        <button
          key={s.label}
          className="flex-1 flex flex-col items-center justify-center gap-1 px-4 py-3 group transition-colors"
          style={{
            ...(i < stats.length - 1 ? { borderRight: '1px solid var(--widget-sep)' } : {}),
            background: 'transparent',
          }}
          onMouseEnter={e => (e.currentTarget.style.background = 'var(--widget-hover)')}
...
function AgentsWidget() {
  ...
  return (
    <Widget title="Agents" icon={<Bot className="w-3.5 h-3.5" />} onClick={() => openWindow(APP_MAP.agents)} className="h-full">
          ...
          (agents as any[]).map((agent: any) => {
            ...
            return (
              <div
                ...
                onMouseEnter={e => { if (!active) e.currentTarget.style.background = 'var(--widget-hover)' }}
                onMouseLeave={e => { if (!active) e.currentTarget.style.background = 'transparent' }}
              >
                <div
                  className="w-7 h-7 rounded-lg flex items-center justify-center text-[11px] font-bold flex-shrink-0"
                  style={{ background: 'var(--widget-avatar-bg)', color: '#c084fc' }}
                >
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  {active > 0 ? (
                    ...
                    <span className="w-1.5 h-1.5 rounded-full" style={{ background: 'var(--widget-idle-dot)' }} />
                  )}
                </div>
              </div>
            )
...
function TasksWidget() {
  ...
  const activeTasks  = (tasks as any[]).filter(t => t.status === 'running' || t.status === 'pending').slice(0, 6)
  const recentDone   = (tasks as any[]).filter(t => t.status === 'completed' || t.status === 'failed').slice(0, 3)
  const display      = [...activeTasks, ...recentDone].slice(0, 7)

  return (
    <Widget title="任务" icon={<ListTodo className="w-3.5 h-3.5" />} onClick={() => openWindow(APP_MAP.tasks)} className="h-full">
      <div className="flex-1 overflow-auto p-2 space-y-1">
        {display.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-24 text-ink-4 gap-2">
...
interface ActivityEvent {
  id: string; time: Date; type: string; label: string; detail?: string; color: string
}
...
function ActivityFeedWidget() {
  ...
  useEffect(() => {
    ...
    const push = (e: Omit<ActivityEvent, 'id' | 'time'>) => {
      setEvents(prev => [{ ...e, id: Math.random().toString(36).slice(2), time: new Date() }, ...prev.slice(0, 49)])
    }

    const onConnect    = () => push({ type: 'connect',    label: '已连接到服务器', color: '#22c55e' })
    const onDisconnect = () => push({ type: 'disconnect', label: '服务器连接断开', color: '#ef4444' })
    const onTaskStatus = (data: any) => {
      const label: Record<string, string> = { running: '任务开始运行', completed: '任务完成', failed: '任务失败', pending: '任务排队中' }
      push({ type: 'task:status', label: label[data.status] ?? `任务: ${data.status}`, detail: data.taskId?.slice(0, 8), color: STATUS_TEXT[data.status] ?? '#94a3b8' })
    }
    const onChatDone = (data: any) => push({ type: 'chat', label: 'Chat 回复完成', detail: data.content?.slice(0, 30), color: '#c084fc' })

    socket.on('connect',     onConnect)
    socket.on('disconnect',  onDisconnect)
    socket.on('task:status', onTaskStatus)
    socket.on('chat:done',   onChatDone)
    if (socket.connected) push({ type: 'connect', label: '已连接到服务器', color: '#22c55e' })
  ...
  return (
    <Widget title="实时动态" icon={<Activity className="w-3.5 h-3.5" />} className="h-full">
        ...
        {events.length === 0 ? (
          ...
          events.map(e => (
            <div
              ...
              onMouseEnter={ev => (ev.currentTarget.style.background = 'var(--widget-row-hover)')}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {e.detail && <div className="text-[10px] text-ink-4 truncate font-mono">{e.detail}</div>}
              </div>
...
function QuickLaunchWidget() {
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={widgetBase}>
      <div className="grid grid-cols-4 gap-px p-1">
        {shortcuts.map(s => (
          <button
            key={s.label}
            className="flex flex-col items-center gap-1.5 px-2 py-3 rounded-xl group transition-all"
...
export function DesktopWidgets() {
  return (
    ...
    >
      {/* Widget 区域：不撑满，从顶部自然向下排列，留出底部空白 */}
      <div className="p-5 flex flex-col gap-3" style={{ pointerEvents: 'auto' }}>
        <SystemOverviewWidget />
        <QuickLaunchWidget />
        {/* 三列 widget 固定高度，不再 flex-1 撑满 */}
        <div className="grid grid-cols-3 gap-3" style={{ height: 260 }}>
          <AgentsWidget />
          <TasksWidget />
          <ActivityFeedWidget />
        </div>
